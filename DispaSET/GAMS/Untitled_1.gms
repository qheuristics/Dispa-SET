...
************************************Modification********************************
find_Q_T(dh,h,t_index)
find_Q_T_bis(dh,h,t_index)
********************************************************************************
;
...
LOAD_HP(dh,h)                 [MW]    Power consumed by heat pumps
Q_dot_chp(dh,h)               [MW]    Heat
Q_dot_dh(dh,h)                [MW]    Heat
Q_dot_gen(dh,h)               [MW]    Heat
Q_dot_hp(dh,h)                [MW]    Heat
Q_sto(dh,h)                   [MW]    Heat
;
********************************************************************************
...
*COP
*===============================================================================
EQ_HP_cop_1(dh,i,t_index)..
         LOAD_HP(dh,i)
         =G=
         find_Q_T(dh,i,t_index)/(COP(dh,t_index))
;
EQ_HP_cop_2(dh,i,t_index)..
         lambda(dh,i,t_index,'1')
         -lambda(dh,i,t_index,'2')
         =E=
         +((Q_dot_hp(dh,i)-T_int(dh,t_index)/(T_chp_out(dh)-T_hp_in(dh)) * Q_dot_gen(dh,i)))$(ord(t_index) >T_hp_in(dh) and ord(t_index) <T_chp_out(dh)+1 )
         +10000$(ord(t_index) <T_hp_in(dh)+1 or ord(t_index) >T_chp_out(dh))

;

EQ_HP_cop_3(dh,i,t_index)..
         find_Q_T(dh,i,t_index)
         =E=
         +(-(lambda(dh,i,t_index,'1')+lambda(dh,i,t_index-1,'2'))*10000+Q_dot_hp(dh,i))$(ord(t_index) >T_hp_in(dh)+1 and ord(t_index) <T_chp_out(dh)+1 )
         +(-(lambda(dh,i,t_index,'1'))*10000+Q_dot_hp(dh,i))$(ord(t_index) =T_hp_in(dh)+1)
         -(1)$(ord(t_index) < T_hp_in(dh)+1)
         -(1)$(ord(t_index) > T_chp_out(dh))
;
*===============================================================================
*HP
*===============================================================================
...
;
EQ_HP_2(dh,i)..
         Q_dot_hp(dh,i)-Q_dot_hp(dh,i-1)$(ord(i) > 1)
         =L=
         HP_RampUp(dh)
;
EQ_HP_3(dh,i)..
         Q_dot_hp(dh,i-1)$(ord(i) > 1)-Q_dot_hp(dh,i)
         =L=
         HP_capacity(dh)
;
...
OuputQ_sto(dh,h)
OutputLOAD_HP(dh,h)
Outputfind_Q_T(dh,h,t_index)
Outputlambda(dh,h,t_index,bin_index)
********************************************************************************
OutputCommitted(u,h)
OutputHeat(chp,h)
OutputFlow(l,h)
...
;
************************************Modification********************************
OuputQ_dot_hp(dh,z)=Q_dot_hp.L(dh,z);
OuputQ_dot_chp(dh,z)=Q_dot_chp.L(dh,z);
OuputQ_dot_gen(dh,z)=Q_dot_gen.L(dh,z);
OuputQ_sto(dh,z)=Q_sto.L(dh,z);
OutputLOAD_HP(dh,z)=LOAD_HP.L(dh,z);
Outputfind_Q_T(dh,z,t_index)=find_Q_T.L(dh,z,t_index);
Outputlambda(dh,z,t_index,bin_index)=lambda.L(dh,z,t_index,bin_index);
********************************************************************************

...
def build_simulation_heat(config,plot_load=False):
    ...
    if not isinstance(config['default']['CostLoadShedding'],(float,int)):
        config['default']['CostLoadShedding'] = 1000
    if not isinstance(config['default']['CostHeatSlack'],(float,int)):
        config['default']['CostHeatSlack'] = 5000
    ...
    check_units(config, plants)
    # If not present, add the non-compulsory fields to the units table:
    for key in ['CHPPowerLossFactor','CHPPowerToHeat','CHPType','STOCapacity','STOSelfDischarge','STOMaxChargingPower','STOChargingEfficiency', 'CHPMaxHeat','HEATType','HEATCapacity','Tmin','Tmax','m','p','DistrictHeating']:
        if key not in plants.columns:
            plants[key] = np.nan
...
####check_heat_demand(plants,.)
    ...
    # Defining the CHPs:
    Plants_chp = Plants_merged[[x.lower() in commons['types_CHP'] for x in Plants_merged['CHPType']]].copy()
##### Defining the genheqt:
    Plants_heat = Plants_merged[[x.lower() in commons['types_HEAT'] for x in Plants_merged['HEATType']]].copy()
    # check chp plants:
    check_chp(config, Plants_chp)
    # For all the chp plants correct the PowerCapacity, which is defined in cogeneration mode in the inputs and in power generation model in the optimization model
    for u in Plants_chp.index:
...
#%% District heating  %% #      
    #Heat demand
    HeatDemandTot = NodeBasedTable(config['HeatDemand'],idx_utc_noloc,config['countries'],tablename='HeatDemand')
    if config['DistrictHeating']['DemandModifier'] != 1:
        logging.info('Scaling HeatDemand curve by a factor ' + str(config['DistrictHeating']['DemandModifier']))
        HeatDemandTot = HeatDemandTot * config['DistrictHeating']['DemandModifier']
    #Part of district heating demand in total heat demand
    #initialization of table
    DistricHeatingDemand = HeatDemandTot 
    PersonalHeatingDemand = HeatDemandTot 
    logging.info('District heating demand represent ' + str(config['DistrictHeating']['PartDH'])+' of total heat demand')
    DHDemand = HeatDemandTot *config['DistrictHeating']['PartDH']
    PHDemand = HeatDemandTot *(1-config['DistrictHeating']['PartDH'])
    ...
    #Temperature
    T_out = NodeBasedTable(config['DistrictHeating']['T_out'],idx_utc_noloc,config['countries'],tablename='T_out')
...
##########################################    
###### List of parameters whose value is known, and provided in the dataframe Plants_heat
    #hourly heat demand parameters
    ...
    for i in range(len(sets['n'])):
        values[:] = CostHeatSlack[sets['n'][i]]
    parameters['CostHeatSlack'] = {'sets': sets_param['CostHeatSlack'], 'val': values}
...
#########################################
    ...
    # Simulation data:
    SimData_heat = {'sets': sets, 'parameters': parameters, 'config': config, 'units': Plants_merged, 'version': dispa_version}

    # list_vars = []
    gdx_out = "Inputs_heat.gdx"
    if config['WriteGDX']:
        write_variables(config['GAMS_folder'], gdx_out, [sets, parameters])

    # if the sim variable was not defined:
    if 'sim' not in locals():
        logging.error('Please provide a path where to store the DispaSET inputs_heat (in the "sim" variable)')
        sys.exit(1)
    ...
    if config['WritePickle']:
        try:
            ...
            import pickle
        with open(os.path.join(sim, 'Inputs_heat.p'), 'wb') as pfile:
            pickle.dump(SimData_heat, pfile, protocol=pickle.HIGHEST_PROTOCOL)
    logging.info('Build finished')

...
from .data_handler import UnitBasedTable,DHBasedTable,NodeBasedTable,merge_series, define_parameter, write_to_excel, load_csv
...
##_______________________________________________________________________________
 ...
 #__________________________%% Modification %% __________________________________
    ...
    # PV scaling
    if config_heat['Portofolio']['Solar']:
        a=0
    ...
    if config_heat['Portofolio']['Winon']:
        a=0
        for u in Plants_merged.index:
            if Plants_merged.Technology[u] == 'WTON':
                a=a+Plants_merged.loc[u, 'PowerCapacity']   
        logging.info('Modifying Solar Capacity to the value ' + str(config_heat['Portofolio']['Winon']))
        for u in Plants_merged.index:
            if Plants_merged.Technology[u] == 'WTON':
                if a!=0:
    ...
    if config_heat['Portofolio']['Winoff']:
        a=0
        for u in Plants_merged.index:
            if Plants_merged.Technology[u] == 'WTOF':
                a=a+Plants_merged.loc[u, 'PowerCapacity']   
        logging.info('Modifying Solar Capacity to the value ' + str(config_heat['Portofolio']['Winoff']))
        for u in Plants_merged.index:
            if Plants_merged.Technology[u] == 'WTOF':
                if a!=0:
    ...
    # Biomass scaling
    if config_heat['Portofolio']['Biomass']:
        a=0
        for u in Plants_merged.index:
            if Plants_merged.Fuel[u] == 'BIO':
                a=a+Plants_merged.loc[u, 'PowerCapacity']   
        logging.info('Modifying Solar Capacity to the value ' + str(config_heat['Portofolio']['Biomass']))
        for u in Plants_merged.index:
            if Plants_merged.Fuel[u] == 'BIO':
                if a!=0:
    ...
    # Gas scaling
    if config_heat['Portofolio']['Gas']:
        a=0
        for u in Plants_merged.index:
            if Plants_merged.Fuel[u] == 'GAS':
                a=a+Plants_merged.loc[u, 'PowerCapacity']   
        logging.info('Modifying Solar Capacity to the value ' + str(config_heat['Portofolio']['Gas']))
        for u in Plants_merged.index:
            if Plants_merged.Fuel[u] == 'GAS':
                if a!=0:
    ...
    # Coal scaling
    if config_heat['Portofolio']['Coal']:
        a=0
        for u in Plants_merged.index:
            if Plants_merged.Fuel[u] == 'HRD':
                a=a+Plants_merged.loc[u, 'PowerCapacity']   
        logging.info('Modifying Solar Capacity to the value ' + str(config_heat['Portofolio']['Coal']))
        for u in Plants_merged.index:
            if Plants_merged.Fuel[u] == 'HRD':
                if a!=0:
                    Plants_merged.loc[u, 'PowerCapacity'] = Plants_merged.loc[u, 'PowerCapacity'] * (config_heat['Portofolio']['Coal']/a)
                else:
                    Plants_merged.loc[u, 'PowerCapacity'] =  config_heat['Portofolio']['Coal']
    # Oil scaling
    if config_heat['Portofolio']['Oil']:
        a=0
        for u in Plants_merged.index:
            if Plants_merged.Fuel[u] == 'OIL':
                a=a+Plants_merged.loc[u, 'PowerCapacity']   
        logging.info('Modifying Solar Capacity to the value ' + str(config_heat['Portofolio']['Oil']))
        for u in Plants_merged.index:
            if Plants_merged.Fuel[u] == 'OIL':
                if a!=0:
                    Plants_merged.loc[u, 'PowerCapacity'] = Plants_merged.loc[u, 'PowerCapacity'] * (config_heat['Portofolio']['Oil']/a)
    ...
    # Nuclear scaling
    if config_heat['Portofolio']['Nuclear']:
        a=0
        for u in Plants_merged.index:
            if Plants_merged.Fuel[u] == 'NUC':
                a=a+Plants_merged.loc[u, 'PowerCapacity']   
        logging.info('Modifying Solar Capacity to the value ' + str(config_heat['Portofolio']['Nuclear']))
        for u in Plants_merged.index:
            if Plants_merged.Fuel[u] == 'NUC':
                if a!=0:
...
#_______________________________________________________________________________       
 ...
 #########################%% District heating  %% ################################# 
    ...
    T_outDH = DHBasedTable(DHplants,config_heat['T_outDH'],idx_utc_noloc,config['countries'],tablename='T_outDH')
    print(T_outDH)
    #Va falloir reussir a merger les heat demand

#    print(HeatDemandDH['Amsterdam'])
 #########################%% Temperature  %% #################################
    
    T_supply =DHplants['T_hot_water'] + DHplants['T_pinch']
    T_return =DHplants['T_space_heating'] + DHplants['T_pinch']
    T_hp_in = T_return
    T_chp_out = DHplants['T_CHP_out']
    
    #  V

#########################%% Heat pumps  %% #################################
    #COP
#    COP = DHBasedTable(DHplants,config_heat['COP'],idx_utc_noloc,config['countries'],tablename='COP')
     
    COP_all = pd.read_csv(config_heat['COP'], header=0, skiprows=[], skip_footer=0, index_col=None, parse_dates=False)
    COP=[[i for i in range(len(DHplants))] for j in range(len(COP_all))]
    for u in DHplants.index:
        COP[u]=COP_all[DHplants['HPModel'][u]]
#    print(COP[1])
    
    T_index=[i for i in range(len(COP_all))]
#    print(T_index)
    T_int=np.full((len(DHplants), len(COP_all)), -200)
    for u in DHplants.index:
        for t in range(T_chp_out[u]-T_hp_in[u]):
            T_int[u][t+T_hp_in[u]]=t
#    print(T_int)
    print('stop it')
...
#    COP_fix=T_outDH
    ...
    for u in DHplants.index:
        ...
        print(int(T_outDH[u].value))
    for u in DHplants.index:
        if [DHplants['T_HP_source'][u]]:
            COP_fix[u] =COP[u][DHplants['T_HP_out'][u]-DHplants['T_HP_source'][u]] 
        else:
            COP_fix[u] =COP[u][DHplants['T_HP_out'][u]-int(T_outDH[u])]
    print(COP_fix)
            
            
    #T_HP_out -- if exist,make if statement or something (DH)
...
#########################%% Thermal Storage  %% #################################
 ...
 #########################%% District heating  %% #################################      
    #Heat demand
#    if config_heat['DistrictHeating']['DemandModifier'] != 1:
#        logging.info('Scaling HeatDemand curve by a factor ' + str(config_heat['DistrictHeating']['DemandModifier']))
#        HeatDemandTot = HeatDemandTot * config_heat['DistrictHeating']['DemandModifier']
    #Part of district heating demand in total heat demand
    #initialization of table
    DistricHeatingDemand = HeatDemandDH
    PersonalHeatingDemand = HeatDemandDH 
    logging.info('District heating demand represent ' + str(config_heat['modifiers']['PartDH'])+' of total heat demand')
    DHDemand = HeatDemandDH *config_heat['modifiers']['PartDH']
    PHDemand = HeatDemandDH *(1-config_heat['modifiers']['PartDH'])
    
    #defining heat slack as non unit based
#    CostHeatSlack = NodeBasedTable(config['CostHeatSlack'],idx_utc_noloc,config['countries'],tablename='CostHeatSlack',default=config['default']['CostHeatSlack'])
  
########################### Temperature #############################
#    T_out = NodeBasedTable(config_heat['Temperature']['T_out'],idx_utc_noloc,config['countries'],tablename='T_out')
    T_pinch=[]
    T_hw=[]
    T_sh=[]
    T_supply=[]
    ...
    T_hp_ev=[]
    for u in DHplants.index:
        T_pinch.append(DHplants.loc[u,'T_pinch'])
        T_hw.append(DHplants.loc[u,'T_hot_water'])
        T_sh.append(DHplants.loc[u,'T_space_heating'])
        T_chp.append(DHplants.loc[u,'T_CHP_out'])
        T_hp_out.append(DHplants.loc[u,'T_HP_out'])
        T_hp_ev.append(DHplants.loc[u,'T_HP_source'])
    print(T_hw)
    print('coucou')
#    T_hw=T_out-T_out+config_heat['Temperature']['T_HW']
#    T_sh=T_out
#    T_sh=T_out-T_out+config_heat['Temperature']['T_SH']
#    T_supply=T_hw + config_heat['Temperature']['pinch']
#    T_return=T_sh+ config_heat['Temperature']['pinch']
#    T_chp=T_out
#    T_chp=T_out-T_out+config_heat['Temperature']['T_CHP']
#    T_hp_in=T_return
#    cp_w=T_out
#    cp_w=T_out-T_out+4185
#    T_hp_ev=T_out
#    T_hp_ev=T_out-T_out+config_heat['Temperature']['T_HP_ev']

##############################  COP  ###################################################
#    sets['t'] = [str(x + 1) for x in range(config_heat['Temperature']['T_CHP']-config_heat['Temperature']['T_SH']+config_heat['Temperature']['pinch'])]    
    T_int=np.arange(0,-config_heat['Temperature']['T_SH']-config_heat['Temperature']['pinch']+config_heat['Temperature']['T_CHP']+1,dtype=int)
    bin_index=np.arange(0,1)
    T_hp_out=T_int+config_heat['Temperature']['pinch']+config_heat['Temperature']['T_SH']

    T_diff=T_chp-T_hp_in
    print(T_diff.values[1])

    COP=(273.15+T_hp_out)/(T_hp_out-10)*config_heat['Temperature']['eta_cop']
...
#    COP = COP.reindex(idx_long, method='nearest').fillna(method='bfill')
    ...
    T_hp_in = T_hp_in.reindex(idx_long, method='nearest').fillna(method='bfill')
...
#    sets['heat_gen'] = Plants_heat.index.tolist()
    ...
    sets['t_index'] = [str(x + 1) for x in range(T_diff.values[1]+1)] #Index of the size equal to the temperature difference in degree. 
    sets['bin_index']=[str(x + 1) for x in range(2)] #Index for the SOS1 variable for the COP in GAMS
 #_______________________________________________________________________________
    ...
    sets_param = {}
 #__________________________%% Modification %% __________________________________
    sets_param['DHDemand'] = ['h']
    sets_param['PHDemand'] = ['h']
    sets_param['T_out'] = ['h']
    sets_param['COP'] = ['t_index']
    sets_param['T_int'] = ['t_index']
    sets_param['T_diff'] = ['h']
    
    sets_param['cp_w'] = ['h']
    sets_param['T_hw'] = ['h']
    sets_param['T_sh'] = ['h']
    sets_param['T_supply'] = ['h']
    sets_param['T_return'] = ['h']
    sets_param['T_chp'] = ['h']
    sets_param['T_hp_in'] = ['h']
       
    sets_param['m_dot'] = ['h']
    sets_param['HEATCapacity'] = ['heat_gen']
    sets_param['Tmin'] = ['heat_gen']
    sets_param['Tmax'] = ['heat_gen']
#    sets_param['DHLoss'] = ['one']
    sets_param['coeffM'] = ['heat_gen']
    sets_param['coeffP'] = ['heat_gen']
    sets_param['DHIndex'] = ['heat_gen']
#    sets_param['cop'] = ['heat_gen']
 #_______________________________________________________________________________
 ...
 #%%__________________________%% Modification %% __________________________________
    values = np.ndarray([len(sets['h'])])
    for i in range(len(sets['n'])):
        values[:] = PHDemand[sets['n'][i]]
    parameters['PHDemand'] = {'sets': sets_param['PHDemand'], 'val': values}
    values = np.ndarray([len(sets['h'])])
    for i in range(len(sets['n'])):
        values[:] = DHDemand[sets['n'][i]]
    parameters['DHDemand'] = {'sets': sets_param['DHDemand'], 'val': values}

    values = np.ndarray([len(sets['h'])])
    for i in range(len(sets['n'])):
        values[:] = T_out[sets['n'][i]]
    parameters['T_out'] = {'sets': sets_param['T_out'], 'val': values}
    
    values = np.ndarray([len(sets['t_index'])])
    parameters['COP'] = {'sets': sets_param['COP'], 'val': COP}
    
    values = np.ndarray([len(sets['t_index'])])
    parameters['T_int'] = {'sets': sets_param['T_int'], 'val': T_int}
    
    
    values = np.ndarray([len(sets['h'])])
    for i in range(len(sets['n'])):
        values[:] = T_diff[sets['n'][i]]
    parameters['T_diff'] = {'sets': sets_param['T_diff'], 'val': values}
        
    values = np.ndarray([len(sets['h'])])
    for i in range(len(sets['n'])):
        values[:] = cp_w[sets['n'][i]]
    parameters['cp_w'] = {'sets': sets_param['cp_w'], 'val': values}
   
    values = np.ndarray([len(sets['h'])])
    for i in range(len(sets['n'])):
        values[:] = T_hw[sets['n'][i]]
    parameters['T_hw'] = {'sets': sets_param['T_hw'], 'val': values}
    values = np.ndarray([len(sets['h'])])
    for i in range(len(sets['n'])):
        values[:] = T_sh[sets['n'][i]]
    parameters['T_sh'] = {'sets': sets_param['T_sh'], 'val': values}
    values = np.ndarray([len(sets['h'])])
    for i in range(len(sets['n'])):
        values[:] = T_supply[sets['n'][i]]
    parameters['T_supply'] = {'sets': sets_param['T_supply'], 'val': values}
    values = np.ndarray([len(sets['h'])])
    for i in range(len(sets['n'])):
        values[:] = T_return[sets['n'][i]]
    parameters['T_return'] = {'sets': sets_param['T_return'], 'val': values}
    values = np.ndarray([len(sets['h'])])
    for i in range(len(sets['n'])):
        values[:] = T_chp[sets['n'][i]]
    parameters['T_chp'] = {'sets': sets_param['T_chp'], 'val': values}
    values = np.ndarray([len(sets['h'])])
    for i in range(len(sets['n'])):
        values[:] = T_hp_in[sets['n'][i]]
    parameters['T_hp_in'] = {'sets': sets_param['T_hp_in'], 'val': values}
    

    #Heat gen parameters
#    values = np.ndarray([len(sets['h'])])
#    for var in ['HEATCapacity','Tmin','Tmax','coeffM','coeffP','DHIndex']:
#        parameters[var]['val'] = Plants_heat[var].values   
